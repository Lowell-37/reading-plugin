...
export const CHAPTER_AI_ACTIONS = {
    summary: '章节摘要',
    keyPoints: '核心观点',
    characters: '人物和事件',
    timeline: '时间线',
...
export function compactAiText(value, maxLength = 24000) {
...
export function normalizeAiEndpoint(value) {
    ...
    if (!raw)
        throw new AiRequestError('请先填写 AI 接口地址');
    let url;
    try {
        url = new URL(raw);
    }
    ...
    if (!['https:', 'http:'].includes(url.protocol))
        throw new AiRequestError('AI 接口只支持 HTTPS 或 HTTP');
    url.hash = '';
    url.search = '';
...
function readMessageContent(data) {
...
export async function streamAiCompletion({ endpoint, apiKey = '', model, messages, signal, onChunk = () => { }, }) {
    if (!String(model || '').trim())
        throw new AiRequestError('请先填写模型名称');
    ...
    if (!response.body || !contentType.includes('text/event-stream')) {
        const result = readMessageContent(await response.json());
        if (!result)
            throw new AiRequestError('AI 接口没有返回可显示的内容');
        onChunk(result);
        return result;
    }
    ...
    while (true) {
        ...
        for (const line of lines) {
            ...
            if (!payload || payload === '[DONE]')
                continue;
            try {
                const chunk = readDeltaContent(JSON.parse(payload));
                if (!chunk)
                    continue;
                result += chunk;
                onChunk(chunk);
