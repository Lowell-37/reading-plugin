...
export function resolveChangedTextQuoteAnchor(source, anchor, preferredOffset = null, options = {}) {
    ...
    const starts = fuzzyCandidateStarts(normalized.text, needle, regionStart, regionEnd, maxDelta, preferredIndex);
    const contextSize = (anchor.prefix?.length || 0) + (anchor.suffix?.length || 0);
    ...
    for (const start of starts) {
        ...
        for (let length = minimumLength; length <= maximumLength; length += 1) {
            ...
            if (Math.abs(needle.length - length) > maximumDistance)
                continue;
            ...
            if (score >= minimumConfidence)
                candidates.push({ start, length, score });
        }
    }
    if (!candidates.length)
        return null;
    ...
    if (winner.score < minimumConfidence || (runnerUp && winner.score - runnerUp.score < minimumLead))
        return null;
    const result = toSourceResolution(normalized.offsets, winner.start, winner.length, 'fuzzy');
    return { ...result, confidence: winner.score };
}
function fuzzyCandidateStarts(source, needle, regionStart, regionEnd, maxDelta, preferredIndex) {
    const starts = new Set();
    const addAround = (value, radius = maxDelta) => {
    ...
    if (preferredIndex != null)
        addAround(preferredIndex, maxDelta * 2);
    if (needle.length < 8) {
        for (let start = regionStart; start < regionEnd; start += 1)
            starts.add(start);
        return [...starts];
    }
...
function boundedLevenshtein(left, right, maximumDistance) {
