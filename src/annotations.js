export function createAnnotation({ kind, locator, text, note = '', color = '#f4c95d', rects = [], page = null, section = null }) {
...
export function normalizeAnnotations(value) {
  ...
  return value.filter(item => item && item.id && item.kind && (item.locator || item.page))
}
