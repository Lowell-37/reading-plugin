...
const chapterCount = 48
const repeatedSentence = 'This deliberately long paragraph exercises continuous reading, measured placeholders, and progress persistence after a browser restart.'
const chapters = Array.from({ length: chapterCount }, (_, index) => {
  const number = String(index + 1).padStart(3, '0')
  const paragraphs = Array.from({ length: 36 }, (_, paragraphIndex) =>
    `<p>STRESS-SECTION-${number} PARAGRAPH-${String(paragraphIndex + 1).padStart(2, '0')}. ${repeatedSentence}</p>`).join('')
  return chapter(`Stress section ${number}`, paragraphs)
})
...
const navItems = chapters.map((_, index) =>
  `<li><a href="chapters/c${index + 1}.xhtml">Stress section ${index + 1}</a></li>`).join('')
...
<package version="3.0" unique-identifier="book-id" xmlns="http://www.idpf.org/2007/opf">
  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/">
    <dc:identifier id="book-id">quiet-reader-large-reading</dc:identifier>
    <dc:title>Large Reading Stress</dc:title>
...
<head><title>Contents</title></head><body><nav epub:type="toc"><ol>${navItems}</ol></nav></body></html>`)],
  ['OEBPS/styles.css', encoder.encode('body{font-family:serif;line-height:1.7} p{margin:0 0 1em}')],
])
